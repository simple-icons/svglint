 ...
 */
export type Config = {
    [attr: string]: boolean | string | RegExp | string[] | undefined;
    "rule::selector"?: string;
    "rule::whitelist"?: boolean;
};
...
function executeOnElm(
    ...
    if (config["rule::whitelist"]) {
        ...
        if (remaining.length) {
            reporter.error(
                `Found extra attributes ${JSON.stringify(
                    remaining
...
export default function generate(config: Config) {
     ...
     * @param The reporter to report warnings/errors to
     ...
     */
    return function AttrRule(reporter: Reporter, $: Cheerio, ast: AST) {
